...
def select_principal_isoform(genes_df: pl.DataFrame, cds_df: pl.DataFrame) -> pl.DataFrame:
    ...
    principal_isoforms = (
        ...
        .sort(['cds_length', 'Gene stable ID', 'Transcript stable ID'],  # ← FIXED
        ...
        .agg([
            ...
            pl.first('Gene description').alias('gene_description'),  # ← FIXED
            pl.first('Genome name').alias('genome_name'),  # ← FIXED
            pl.first('Transcript biotype').alias('transcript_biotype')  # ← FIXED
        ])
    )
    return principal_isoforms.sort(['Gene stable ID'])  # ← ADD THIS LINE
