...
EXPECTED_COLUMNS = {
    ...
    'Genome name',
    'Gene description',     # ← Changed from 'description'
    'Transcript biotype'    # ← Changed from 'transcript_biotype'
}

CDS_EXPECTED_COLUMNS = {
    'Gene stable ID',
    'Transcript stable ID', 
    'CDS start (within cDNA)',    # ← BioMart returns this
    'CDS end (within cDNA)'       # ← BioMart returns this
}
...
def fetch_with_retry(xml_query: str, expected_columns: Set[str], timeout: int = REQUEST_TIMEOUT) -> pl.DataFrame:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            df = fetch_wbp_biomart_using_xml_polars(xml_query, timeout=timeout)
    ...
    print("[headers]", list(df.columns))  # <-- add this line
...
def select_principal_isoform(genes_df: pl.DataFrame, cds_df: pl.DataFrame) -> pl.DataFrame:
    ...
    principal_isoforms = (
        ...
        .agg([
            ...
            pl.first('Gene description').alias('gene_name'),           # ← FIXED
            # Remove 'description' line since it doesn't exist
            pl.first('Genome name').alias('genome_name'),
            pl.first('Transcript biotype').alias('transcript_biotype') # ← Also fix this
        ])
...
def fetch_with_retry(xml_query: str, expected_columns: Set[str]) -> pl.DataFrame:
    df = fetch_biomart_with_validation(xml_query, expected_columns, REQUEST_TIMEOUT)
    
    # TEMPORARY: Add this print to see what columns BioMart returns
    print("=== DEBUG COLUMNS ===")
    print("Expected:", expected_columns)
    print("Actual:", set(df.columns))
    print("=====================")
    
    # Then the existing validation happens here...
    validate_biomart_columns(df, expected_columns)
    return df
