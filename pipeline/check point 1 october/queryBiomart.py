...
def fetch_wbp_biomart(
    ...
    # Ensure headers are enabled for consistent parsing
    xml_query = _ensure_headers_enabled(xml_query)
...
def _ensure_headers_enabled(xml_query: str) -> str:
    """Ensure header='1' is set in the XML query for consistent parsing."""
    # Remove newlines for clean URL encoding
    xml_query = xml_query.replace('\n', '').replace('\r', '')
    
    # Ensure headers are enabled
    if 'header="0"' in xml_query:
        xml_query = xml_query.replace('header="0"', 'header="1"')
    elif 'header="1"' not in xml_query:
        # Add header attribute if missing
        xml_query = re.sub(r'(<Query[^>]*)>', r'\1 header="1">', xml_query)
