...
def queryWbpHumanOrthologues(genomes):
    ...
    df = df.sample(frac=1).reset_index(drop=True)  # TEST ONLY: shuffle row order

    #temporary block

        # --- DIAGNOSTIC: show ties on max % identity ---
    mx = df.groupby('Gene stable ID')['% identity'].transform('max')
    is_tie_row = df['% identity'].eq(mx)
    ties = (df[is_tie_row]
            .groupby('Gene stable ID', as_index=False)
            .size()
            .query('size > 1'))
    print(f"[diagnose] Genes with tied max %% identity: {len(ties)}")
    print(ties.head(20))
    # -----------------------------------------------
